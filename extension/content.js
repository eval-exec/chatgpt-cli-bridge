...
function sendQuery(text) {
  ...
  if (stopButton) {
    console.log('[ChatGPT CLI Bridge] ChatGPT is busy generating. Stopping current generation first...');
    stopButton.click();
  ...
  const editor = document.querySelector('div[contenteditable="true"]#prompt-textarea, div[contenteditable="true"]');
  ...
  console.log('[ChatGPT CLI Bridge] Text set, waiting for send button to appear...');
  ...
  setTimeout(() => {
    ...
    const checkButton = setInterval(() => {
      ...
      console.log(`[ChatGPT CLI Bridge] Checking for button (attempt ${attempts})...`);
    ...
    const isLoading = document.querySelector('[class*="loading"], [class*="generating"], [data-testid*="loading"]');
    const stopButton = buttons.find(btn => {
      const ariaLabel = btn.getAttribute('aria-label') || '';
      return ariaLabel.toLowerCase().includes('stop');
    });
    ...
    if (sendButton) {
      ...
      const checkTimeout = () => {
        ...
        const loadingIndicators = document.querySelectorAll('[class*="loading"], [class*="spinner"], [class*="animate"], svg[class*="animate"]');
        const isProcessing = loadingIndicators.length > 0 || document.querySelector('button[aria-label*="Stop"]') !== null;

        if (isProcessing && timeoutAttempts < 6) {
          // Still processing, wait another 10 seconds (up to 60 seconds total)
          console.log(`[ChatGPT CLI Bridge] ChatGPT still processing, waiting... (${timeoutAttempts * 10}s)`);
          ...
          console.log('[ChatGPT CLI Bridge] ChatGPT stuck or timeout reached, stopping generation...');
          ...
          // Clear text box
          const editorToClear = document.querySelector('div[contenteditable="true"]#prompt-textarea, div[contenteditable="true"]');
          if (editorToClear) {
            editorToClear.innerHTML = '';
            editorToClear.blur();
          }

          sendError(`ChatGPT did not respond within ${timeoutAttempts * 10} seconds. Refreshing page...`);
      ...
      clearInterval(checkButton);
      console.log('[ChatGPT CLI Bridge] Query already sent, ChatGPT is generating response...');
      ...
      console.log('[ChatGPT CLI Bridge] Could not find send button, clearing text box...');
      ...
      sendError('Could not find send button after ' + maxAttempts + ' attempts. Text box cleared. Please try again.');
...
function startMonitoring() {
  ...
  const checkInterval = setInterval(() => {
    ...
    const container = document.querySelector('main, [class*="conversation"], [role="main"]');
...
function setupObserver(container) {
  ...
  observer = new MutationObserver((mutations) => {
    ...
    const copyButton = latestMessage.querySelector('button[aria-label*="Copy"], button[title*="Copy"]');
    const shareButton = latestMessage.querySelector('button[aria-label*="Share"], button[title*="Share"]');
    ...
    if (messageText !== currentResponseText) {
      ...
      if (!completionCheckInterval && newContent.length > 0) {
        console.log('[ChatGPT CLI Bridge] Starting completion check polling...');
      ...
      if (hasActionButtons) {
        console.log('[ChatGPT CLI Bridge] Response complete (Copy/Share buttons detected)');
